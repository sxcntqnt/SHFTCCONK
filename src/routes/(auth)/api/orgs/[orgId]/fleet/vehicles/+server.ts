...
export const GET: RequestHandler = async ({ params, url, locals }) => {
  ...
  const [vehiclesResult, groupsResult] = await Promise.all([
    supabase
      .from('vehicles')
      .select('id, name, registration, model, chassis, status, vehicle_groups(name)')
      .eq('organization_id', orgId)
...
export const POST: RequestHandler = async ({ params, request, locals }) => {
  ...
  if (action === 'linkDevice') {
    ...
    if (!vehicleId || !deviceId) error(400, 'vehicleId and deviceId are required')
  ...
  const {
    registration, name, model, chassis, engine,
    manufactured_by, vehicle_type, color, registration_expiry,
    group_id,
    // device linking
    device_id, api_url, api_username, api_password,
  } = body
  ...
  // Or create a new device from raw credentials
    ...
    if (deviceErr) console.error('[vehicles +server] device create error:', deviceErr)
  }
