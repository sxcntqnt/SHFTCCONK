...
const ACTION_MAP: Record<string, { chaincode: string; fn: string; requiredRole: string[] }> = {
  // fleet
  AssignDriver:         { chaincode: 'fleet-contract', fn: 'AssignDriver',         requiredRole: ['fleet-manager', 'org-admin'] },
  UpdateVehicleStatus:  { chaincode: 'fleet-contract', fn: 'UpdateVehicleStatus',  requiredRole: ['fleet-manager', 'org-admin'] },
  LogTripCompletion:    { chaincode: 'fleet-contract', fn: 'LogTripCompletion',    requiredRole: ['driver', 'fleet-manager'] },

  // drivers
  UpdateDriverStatus:   { chaincode: 'fleet-contract', fn: 'UpdateDriverStatus',   requiredRole: ['fleet-manager', 'org-admin'] },
  AssignVehicle:        { chaincode: 'fleet-contract', fn: 'AssignVehicle',        requiredRole: ['fleet-manager', 'org-admin'] },

  // finance / wallet
  RecordWalletTopUp:       { chaincode: 'finance-contract', fn: 'RecordWalletTopUp',       requiredRole: ['org-admin', 'fleet-manager'] },
  ReconcileTripPayment:    { chaincode: 'finance-contract', fn: 'ReconcileTripPayment',    requiredRole: ['fleet-manager', 'org-admin'] },

  // compliance
  LogComplianceEvent:       { chaincode: 'compliance-contract', fn: 'LogComplianceEvent',       requiredRole: ['fleet-manager', 'org-admin', 'iot-device'] },
  UpdateComplianceStatus:   { chaincode: 'compliance-contract', fn: 'UpdateComplianceStatus',   requiredRole: ['fleet-manager', 'org-admin'] },

  // devices (pair flow — after device is already enrolled at CA level by admin)
  RegisterDeviceOnChain:  { chaincode: 'fleet-contract', fn: 'RegisterDeviceOnChain',  requiredRole: ['org-admin', 'fleet-manager'] },
  UpdateDeviceLocation:   { chaincode: 'fleet-contract', fn: 'UpdateDeviceLocation',   requiredRole: ['iot-device', 'fleet-manager'] },
};
...
export const POST: RequestHandler = async ({ request, params, locals }) => {
  ...
  if (!mapped) {
    throw error(400, `Unknown action: "${action}". Allowed: ${Object.keys(ACTION_MAP).join(', ')}`);
  }
