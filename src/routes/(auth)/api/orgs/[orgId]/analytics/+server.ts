...
export const GET: RequestHandler = async ({ url, locals }) => {
  ...
  const [routeResult, revenueResult, fleetResult] = await Promise.allSettled([
    ...
    fetchRoutes
      ? supabase
          ...
          .then(({ data, error }) => {
            if (error) throw new Error(`Route analytics fetch failed: ${error.message}`)
            return (data ?? []).map((row) => mapRouteStats(row as Record<string, unknown>))
          })
    ...
    fetchRevenue
      ? supabase
          ...
          .select("total_collected, expected_amount, variance, status, created_at")
          .eq("organization_id", orgId)
          .gte("created_at", from ?? new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString())
          .lte("created_at", to   ?? new Date().toISOString())
          .order("created_at", { ascending: true })
          .then(({ data, error }) => {
            if (error) throw new Error(`Revenue trend fetch failed: ${error.message}`)
    ...
    fetchFleet
      ? supabase
          ...
          .eq("organization_id", orgId)
          .then(({ data, error }) => {
            if (error) throw new Error(`Fleet utilisation fetch failed: ${error.message}`)
  ...
  // ── Extract results ───────────────────────────────────────────────────────
  const routeStats    = routeResult.status  === "fulfilled" ? routeResult.value  : null
  const revenueTrend  = revenueResult.status === "fulfilled" ? revenueResult.value : null
  const fleetStats    = fleetResult.status  === "fulfilled" ? fleetResult.value  : null

  // Log failures but don't hard-fail — return partial data
  if (routeResult.status  === "rejected") console.error("[analytics] routes:", routeResult.reason)
  if (revenueResult.status === "rejected") console.error("[analytics] revenue:", revenueResult.reason)
  if (fleetResult.status  === "rejected") console.error("[analytics] fleet:", fleetResult.reason)

  // ── Single route filter ───────────────────────────────────────────────────
  const filteredRoutes = route && routeStats
...
function buildRevenueTrend(
  rows: { total_collected: number; expected_amount: number; variance: number; status: string; created_at: string }[],
): RevenueTrendPoint[] {
