...
export const POST: RequestHandler = async ({ request }) => {
  ...
  if (
    ...
    return json({ error: "Required: id, org, t, la, lo" }, { status: 400 })
  }
  ...
  if (!shouldBroadcast) {
    try {
      const lastState = await streamClient.hgetall(
      ...
      if (!lastState || !lastState.la) {
        ...
        shouldBroadcast =
          distMetres > MIN_MOVE_METRES || elapsedMs > MIN_BROADCAST_MS
      }
    } catch (err) {
      // Redis read failure — fail open (broadcast anyway)
      console.warn(
        "[gps-ingest] Movement filter read failed, broadcasting:",
        err,
      )
      shouldBroadcast = true
    }
  ...
  writes.push(
    geoClient.call(
      ...
      vehicleId,
      "EX",
      String(LATEST_STATE_TTL), // auto-expire stale positions
      "POINT",
      lat,
      lng,
    ),
  ...
  for (const [i, result] of results.entries()) {
    if (result.status === "rejected") {
      ...
      console.error(
        `[gps-ingest] Write ${labels[i] ?? i} failed:`,
        result.reason,
      )
    }
  }
