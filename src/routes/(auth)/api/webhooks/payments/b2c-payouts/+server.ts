...
//   2. We validate permissions and call Daraja B2C API
...
//   5. The b2c-callback webhook updates status to 'completed' or 'failed'
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  const { phoneNumber, amount, role, actorId, orgId, tripId } = body as Record<string, unknown>;
  ...
  try {
    ...
    const response = await mpesa.sendB2CPayment({
      phoneNumber: phoneNumber as string,
      amount: amountKes,
      remarks: `Tip payout — ${roleLabel}`,
      occasion: `Tip for ${roleLabel.toLowerCase()}`,
    });
    ...
    // Record payout in 'processing' state
