// src/routes/api/webhooks/mpesa/b2b-payouts/+server.ts
//
...
//   2. We validate permissions and call Daraja B2B API
//   3. Daraja returns ConversationID immediately (async)
//   4. We insert a 'processing' row into mpesa_settlements
//   5. The b2b-callback webhook updates status to 'completed' or 'failed'
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  const { shortcode, amount, orgId, reference, actorId } = body as Record<string, unknown>;
  ...
  // Verify actor has jurisdiction over the org
  ...
  if (!membership) {
    return json({ error: "Actor is not a member of this organisation" }, { status: 403 });
  }
  ...
  try {
    ...
    // Record settlement in 'processing' state
