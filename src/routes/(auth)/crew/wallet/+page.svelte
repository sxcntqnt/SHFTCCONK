...
<script lang="ts">
  ...
  const bookingsServed = transactions
    .filter((t: any) => t.type === 'reservation_share')
    .length
...
<svelte:head>
  <title>{crewType === 'DRIVER' ? 'Driver' : 'Conductor'} Wallet — Matatu Pulse</title>
</svelte:head>
...
{#if hlfStatus}
  {#if hlfStatus.canLogOnChain}
    ...
    <div class="mx-6 mt-6 flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-5 py-2.5 text-xs text-emerald-700">
  ...
  {:else if hlfStatus.status === 'pending' || hlfStatus.status === 'retrying'}
    ...
    <div class="mx-6 mt-6 flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-5 py-3 text-sm text-amber-700">
  ...
  {:else if hlfStatus.status === 'exhausted'}
    ...
    <div class="mx-6 mt-6 flex items-start gap-3 rounded-xl border border-rose-200 bg-rose-50 px-5 py-3 text-sm text-rose-700">
      ...
      <div>
        <div class="font-medium">Fabric enrollment failed after {hlfStatus.attempts} attempts</div>
        <div class="text-xs mt-0.5 opacity-80">
          On-chain fare and trip logging is unavailable. Your wallet earnings are unaffected.
          Contact your SACCO administrator to re-trigger enrollment.
        </div>
        {#if hlfStatus.lastError}
          <div class="mt-1.5 rounded bg-rose-100 px-2 py-1 font-mono text-xs opacity-70">
            {hlfStatus.lastError}
          </div>
        {/if}
      </div>
  ...
  {:else}
    <!-- Not enrolled at all -->
    <div class="mx-6 mt-6 flex items-start gap-3 rounded-xl border border-gray-200 bg-gray-50 px-5 py-3 text-sm text-gray-600">
      <svg class="h-4 w-4 mt-0.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>
      </svg>
      <div>
        <div class="font-medium text-gray-700">Fabric identity not yet enrolled</div>
        <div class="text-xs mt-0.5">
          On-chain logging requires a Hyperledger identity. Your SACCO administrator
          can trigger enrollment from the org dashboard.
        </div>
      </div>
    </div>
  {/if}
