...
{:else}
  <div class="space-y-10">
    {#each $contracts as contract (contract.id)}
      <div
        class="bg-white/90 backdrop-blur-xl p-6 rounded-3xl shadow-xl border border-white/60"
      >
        <!-- Header -->
        <div
        ...
        >
          <div>
            ...
            <p class="text-sm text-gray-600">
              Subsidy: <span class="font-semibold"
                >KES {contract.subsidyAmount.toLocaleString()}</span
              >
            </p>
          </div>
          ...
          <div class="w-full md:w-64">
            <div class="text-xs text-gray-600 mb-1">
              Capacity: {contract.assignedVehicles
                .length}/{contract.maxVehicles}
            </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
          {#each $fleet as vehicle (vehicle.id)}
            <button
              ...
              disabled={isVehicleAssigned(contract, vehicle.id) ||
                contract.assignedVehicles.length >= contract.maxVehicles}
