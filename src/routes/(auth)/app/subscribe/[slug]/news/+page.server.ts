...
export const load: PageServerLoad = async ({ params, locals, url }) => {
  ...
  if (!session || !user) {
    throw redirect(303, `/auth/login?redirect=/app/subscribe/${params.slug}/news`)
  }
  ...
  let query = (locals.supabase as any)
    .from('org_news')
    .select('id, title, body, category, severity, pinned, route_ids, author_id, created_at, profiles:author_id ( full_name, avatar_url )')
