...
<script lang="ts">
  ...
  interface Profile {
    ...
    website?:               string
    phone?:                 string
    starting_locations?:    string
    destinations?:          string
    highway_corridors?:     string[]
    routes_to_track?:       string[]
  ...
  let workingHoursStart    = $state(profile?.working_hours_start   ?? '')
  let workingHoursEnd      = $state(profile?.working_hours_end     ?? '')
  let timeZone             = $state(profile?.time_zone             ?? 'Africa/Nairobi')
...
<div class="page">
  <div class="card">
      ...
      <div class="form-body">
        ...
        <div class="field">
          ...
          <input
            ...
            bind:value={fullName} maxlength="50" required
          />
        ...
        <div class="field">
          ...
          <div class="phone-wrap {fieldError('phone') ? 'err' : ''}">
            ...
            <input
              ...
              class="phone-input"
              bind:value={phone} maxlength="15" required
            />
          </div>
          {#if fieldError('phone')}
        ...
        <div class="field">
          ...
          <input
            ...
            class="field-input" bind:value={companyName} maxlength="50"
          />
        </div>
        ...
        <div class="field">
          ...
          <input
            ...
            class="field-input" bind:value={website} maxlength="100"
          />
        </div>
        ...
        <div class="field">
          ...
          <input
            id="startingLocations" name="startingLocations" type="text"
            placeholder="e.g. CBD, Westlands"
            class="field-input" bind:value={startingLocations} maxlength="200"
          />
        </div>
        ...
        <div class="field">
          ...
          <input
            id="destinations" name="destinations" type="text"
            placeholder="e.g. Thika Road, Ngong Road"
            class="field-input" bind:value={destinations} maxlength="200"
          />
        </div>
        ...
        <div class="field">
          ...
          <select
            id="preferredVehicleType" name="preferredVehicleType"
            class="field-input" multiple bind:value={preferredVehicleType}
          >
        ...
        <div class="field">
          ...
          <select
            id="languagesSpoken" name="languagesSpoken"
            class="field-input" multiple bind:value={languagesSpoken}
          >
        ...
        <div class="field">
          ...
          <input
            id="socialMediaLinks" name="socialMediaLinks" type="url"
            placeholder="https://linkedin.com/in/amina-odhiambo"
            class="field-input" bind:value={socialMediaLinks} maxlength="200"
          />
        </div>
        ...
        <div class="field">
          ...
          <input
            id="emergencyContacts" name="emergencyContacts" type="text"
            placeholder="e.g. 0712 345 678, 0722 987 654"
            class="field-input" bind:value={emergencyContacts} maxlength="150"
          />
        </div>
        ...
        <div class="field">
          ...
          <div class="time-range">
            <input
              type="time" id="workingHoursStart" name="workingHoursStart"
              class="field-input time-input" bind:value={workingHoursStart}
            />
            <span class="time-separator">to</span>
            <input
              type="time" id="workingHoursEnd" name="workingHoursEnd"
              class="field-input time-input" bind:value={workingHoursEnd}
            />
          </div>
        </div>
        ...
        <div class="field">
          ...
          <select id="timeZone" name="timeZone" class="field-input" bind:value={timeZone}>
        ...
        <div class="field">
          ...
          {:else}
            <div class="org-search-wrap">
              ...
              />
              {#if orgSearch}
                <button type="button" class="org-clear" onclick={() => orgSearch = ''} aria-label="Clear search">
                  ✕
                </button>
              {/if}
            </div>
            ...
            {#if selectedOrgs.size > 0}
              <div class="selected-pill">
                ...
                <button type="button" class="clear-all" onclick={() => selectedOrgs = new Set()}>
                  Clear all
                </button>
              </div>
            {/if}
    ...
    <div class="card-footer">
      ...
      <a href="/app/sign_out">Sign out</a>
    </div>
  </div>
</div>
...
<style>
  ...
  .subtitle { font-size: 0.82rem; color: var(--text-3); line-height: 1.5; }
  .form-body { padding: 24px 32px; display: flex; flex-direction: column; gap: 18px; }
  ...
  .optional {
    ...
    padding: 1px 5px; border: 1px solid rgba(255,255,255,0.1); border-radius: 4px;
  }
  .field-input {
    ...
    border-radius: 10px;
    font-family: var(--font-body); font-size: 0.88rem; color: var(--text-1);
    transition: border-color 0.15s, background 0.15s;
    box-sizing: border-box;
  }
  .field-input::placeholder { color: var(--text-3); opacity: 0.6; }
  .field-input:focus {
    outline: none;
  ...
  .field-input.err { border-color: rgba(248,113,113,0.5); }
  .field-err { font-size: 0.66rem; color: #f87171; }
  .phone-wrap {
  ...
  .phone-wrap.err { border-color: rgba(248,113,113,0.5); }
  .phone-prefix {
    padding: 10px 12px; font-size: 0.85rem; font-weight: 700;
    color: var(--teal); background: rgba(0,176,155,0.08);
    border-right: 1px solid rgba(255,255,255,0.08);
    display: flex; align-items: center; flex-shrink: 0; user-select: none;
  }
  .phone-input {
    flex: 1; padding: 10px 13px; background: transparent; border: none; outline: none;
    font-family: var(--font-body); font-size: 0.88rem; color: var(--text-1); min-width: 0;
  }
  .phone-input::placeholder { color: var(--text-3); opacity: 0.6; }
  .time-range { display: flex; align-items: center; gap: 10px; }
  .time-input { flex: 1; }
  .time-separator { font-size: 0.78rem; color: var(--text-3); flex-shrink: 0; }
  .org-search-wrap {
  ...
  .org-search {
    ...
    font-family: var(--font-body); font-size: 0.82rem; color: var(--text-1);
  }
  .org-search::placeholder { color: var(--text-3); opacity: 0.6; }
  .org-clear {
    background: none; border: none; color: var(--text-3);
    cursor: pointer; font-size: 0.75rem; padding: 0 2px; opacity: 0.6;
  ...
  .selected-pill {
    ...
    padding: 4px 10px;
    background: rgba(0,176,155,0.09); border: 1px solid rgba(0,176,155,0.2);
    border-radius: 100px; font-size: 0.66rem; font-weight: 700;
    color: var(--teal); margin-bottom: 8px;
  }
  .selected-dot { width: 5px; height: 5px; border-radius: 50%; background: var(--teal); }
  .clear-all {
    background: none; border: none; font-family: var(--font-body);
    font-size: 0.62rem; font-weight: 700; color: var(--text-3);
    cursor: pointer; margin-left: 4px; padding: 0;
    text-decoration: underline; opacity: 0.7; transition: opacity 0.15s;
  }
  ...
  .org-check.checked { background: var(--teal); border-color: var(--teal); color: #fff; }
  ...
  .submit-btn {
    ...
    font-family: var(--font-body); font-size: 0.92rem; font-weight: 700; color: #fff;
    cursor: pointer;
    transition: background 0.15s, transform 0.15s, box-shadow 0.15s;
    box-shadow: 0 4px 18px rgba(0,176,155,0.28);
  }
  .submit-btn:hover:not(:disabled) {
    background: #009a88; transform: translateY(-1px);
  ...
  .card-footer {
    ...
    font-size: 0.72rem; color: var(--text-3); border-top: 1px solid var(--rim);
  }
