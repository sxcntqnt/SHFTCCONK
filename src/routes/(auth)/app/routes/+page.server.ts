...
export const load: PageServerLoad = async ({ locals, url }) => {
  ...
  const trackedOrgIds = [
    ...new Set(trackedRoutes.map((r) => r.organizationId).filter(Boolean)),
  ]
  let stageRoutes: any[] = []
  if (trackedOrgIds.length > 0) {
    try {
      const { data: stages } = await (locals.supabase as any)
        ...
        .select(
          "id, stage_name, route, organization_id, created_at, organizations ( name )",
        )
  ...
  try {
    ...
    if (actorIds.length > 0) {
      const { data: bookings } = await (locals.supabase as any)
        .from("bookings")
        .select(
          "id, route_from, route_to, fare, status, created_at, vehicle_id, vehicles ( reg_number, organization_id, organizations ( name ) )",
        )
