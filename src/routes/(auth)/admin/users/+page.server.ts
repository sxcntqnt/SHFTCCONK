...
export const load: PageServerLoad = async ({ locals, url }) => {
  ...
  const { data: profiles, error: pErr } = await supabase
    .from("profiles")
    .select(
      "id, full_name, company_name, avatar_url, unsubscribed, permissions_version, created_at",
    )
  ...
  const allActorIds = Object.values(actorsByProfile)
    .flat()
    .map((a: any) => a.id)
...
async function sendAtSms(phone: string, message: string): Promise<void> {
  ...
  const failed = (json?.SMSMessageData?.Recipients ?? []).filter(
    (r: any) => r.status !== "Success",
  )
  if (failed.length)
    throw new Error(
      `SMS failed for: ${failed.map((r: any) => r.number).join(", ")}`,
    )
}
...
export const actions: Actions = {
  ...
  send_verification: async ({ request, locals }) => {
    ...
    if (
      method === "sms" &&
      !/^(\+?254|0)[17]\d{8}$/.test(dest.replace(/\s/g, ""))
    ) {
      return fail(400, {
    ...
    if (actor.status === "active")
      return fail(409, { error: "Actor is already verified" })
    if (actor.status === "inactive")
      return fail(409, { error: "Actor is deactivated — reactivate first" })
    ...
    if (method === "sms") {
      ...
      if (insertErr)
        return fail(500, { error: "Could not create verification record" })

      try {
        await sendAtSms(
          dest,
          `Your sxcntqnt verification code is: ${otp}\n\nExpires in 15 minutes. Do not share.`,
        )
      } catch (err) {
        await supabase
          .from("actor_verification_tokens")
      ...
      await supabase.from("audit_logs").insert({
        event_type: "verification_sent_sms",
        actor_id,
        profile_id: actor.profile_id,
        performed_by: user?.id,
        details: { phone: dest.slice(0, 6) + "****" },
      })
    ...
    await supabase.from("audit_logs").insert({
      event_type: "verification_sent_email",
      actor_id,
      profile_id: actor.profile_id,
      performed_by: user?.id,
      details: { email: dest },
  ...
  deactivate_actor: async ({ request, locals }) => {
    ...
    if (actor.type === "SUPER_ADMIN" && actor.profile_id === user?.id)
      return fail(400, {
        error: "Cannot deactivate your own SUPER_ADMIN actor",
      })
