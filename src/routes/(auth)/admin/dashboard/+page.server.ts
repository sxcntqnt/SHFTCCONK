...
export const load: PageServerLoad = async ({ locals }) => {
  ...
  const [
    ...
    supabase
      .from("actor_requests")
      .select(
        `id, requested_type, status, payload, created_at, profiles ( full_name, avatar_url )`,
      )
    ...
    supabase
      .from("audit_logs")
      .select(
        `id, event_type, performed_by, details, created_at, severity, performer:profiles!audit_logs_performed_by_fkey ( full_name )`,
      )
      .order("created_at", { ascending: false })
      .limit(5),
    supabase
