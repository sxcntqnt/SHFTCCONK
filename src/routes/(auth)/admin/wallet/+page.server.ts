...
export const load: PageServerLoad = async ({ locals }) => {
  ...
  const reservationTx: WalletTransaction[] = (bookingsResult.data ?? []).map(b => {
  ...
  // ── Tip payout outflows ────────────────────────────────────────────────────
  const tipOutTx: WalletTransaction[] = (tipPayoutsResult.data ?? []).map(p => ({
    id:          `payout-${p.id}`,
    type:        'b2c_payout',
  ...
  const settlementOutTx: WalletTransaction[] = (settlementsResult.data ?? []).map(s => ({
    id:          `settle-${s.id}`,
    type:        'b2b_settlement',
  ...
  const hlfSummary  = {
    ...
    pending:   queueRows.filter(r => ['pending', 'retrying'].includes(r.status)).length,
