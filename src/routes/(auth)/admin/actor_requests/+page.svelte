<script lang="ts">
  ...
  function needsOrg(type: string) {
    return ["organization_member", "fleet_ownership", "sacco_chair"].includes(
      type,
    )
  }
          ...
          <div class="ar-card" style="animation-delay: {i * 60}ms">
              ...
              <div class="ar-requester">
                ...
                <div class="ar-avatar">
                  {#if r.profiles?.avatar_url}
                    ...
                    />
                  {:else}
                    <span class="ar-avatar-initials"
                      >{initials(r.profiles?.full_name ?? null)}</span
                    >
                  {/if}
                </div>
                <div>
                  ...
                  {#if r.profiles?.company_name}
                    <div class="ar-requester-company">
              ...
              <div class="ar-payload">
                {#each formatPayload(r.payload, data.orgMap) as field}
                  <div class="ar-field">
                    ...
                    <span
                      class="ar-field-value"
                      class:ar-field-mono={field.isId}>{field.value}</span
                    >
                  </div>
                {/each}
              </div>
                  ...
                  <div class="ar-select-wrap">
                    ...
                    >
                      ...
                      <optgroup label="SACCO">
                        <option value="sacco_chair"
                          >SACCO Chairman → Organization</option
                        >
                      ...
                      <div class="ar-select-wrap">
                        ...
                        >
                          ...
                          {#each data.vehicles as v}
                            <option value={v.id}
                              >{v.reg_number} ({v.capacity} seats)</option
                            >
                          {/each}
                        </select>
                        <svg
            ...
            {#if rejectOpen[r.id]}
              <form
                method="post"
                action="?/reject"
                class="ar-reject-form ar-slide-in"
              >
                <input type="hidden" name="request_id" value={r.id} />
                <div class="ar-form-group">
                  <label class="ar-label" for={"reject_reason_" + r.id}
...
<style>
  ...
  .ar-btn-reject-confirm {
    ...
    font-family: "DM Sans", system-ui, sans-serif;
    color: #fff;
    background: linear-gradient(135deg, #dc4444, #b83232);
    border: none;
    border-radius: 10px;
    cursor: pointer;
    transition:
  ...
  .ar-btn-reject-confirm:hover {
    filter: brightness(1.1);
    transform: translateY(-1px);
  }
  .ar-btn-reject-confirm:active {
    transform: translateY(0);
    filter: brightness(0.95);
  }
