...
const VALID_STATUSES = ['pending_activation', 'active', 'suspended', 'inactive'] as const
...
export const load: PageServerLoad = async ({ locals, url }) => {
  ...
  if (orgIds.length > 0) {
    ...
    for (const b of branchRows  ?? []) branchCounts[b.organization_id]  = (branchCounts[b.organization_id]  ?? 0) + 1
...
export const actions: Actions = {
  ...
  create: async ({ request, locals }) => {
    ...
    const status = ((form.get('status') as string) || 'pending_activation') as OrgStatus

    if (!name)                          return fail(400, { error: 'Organization name is required' })
    if (!VALID_STATUSES.includes(status)) return fail(400, { error: 'Invalid status' })
    ...
    if (existing?.length) return fail(409, { error: `An organization named "${name}" already exists` })
  ...
  setStatus: async ({ request, locals }) => {
    ...
    const param = status === 'active' ? 'activated=1' : status === 'suspended' ? 'suspended=1' : 'created=1'
  ...
  delete: async ({ request, locals }) => {
    ...
    if (members?.length) {
      return fail(409, {
        error: 'Cannot delete an organization with active members. Suspend it first.',
      })
    }
