...
export const load: PageServerLoad = async (event) => {
  ...
  const [
    ...
    vehicleIds.length > 0
      ? supabase
          .from("incidents")
          .select(
            "id, vehicle_id, severity, type, description, created_at, metadata",
          )
          .in("vehicle_id", vehicleIds)
          .eq("resolved", false)
  ...
  const trips =
    tripsRes.status === "fulfilled" ? (tripsRes.value.data ?? []) : []
  const revenue =
    revenueRes.status === "fulfilled" ? (revenueRes.value.data ?? []) : []
  const incidents =
    incidentsRes.status === "fulfilled" ? (incidentsRes.value.data ?? []) : []
  const vehicles =
    vehicleStatusRes.status === "fulfilled"
      ? (vehicleStatusRes.value.data ?? [])
      : []
  const pendingCount =
    pendingDispatchRes.status === "fulfilled"
      ? ((pendingDispatchRes.value as { count: number }).count ?? 0)
      : 0

  const todayRevenueKes = revenue.reduce(
    (sum: number, r: { amount: number }) => sum + Number(r.amount ?? 0),
    0,
  )
  const activeVehicles = vehicles.filter(
    (v: { active: boolean; status: string }) =>
  ...
  const recentTrips = trips.slice(0, 12).map((t: Record<string, unknown>) => {
    ...
    return {
      ...
      status:
        (t.status as "IN_PROGRESS" | "COMPLETED" | "DELAYED" | "SCHEDULED") ??
        "SCHEDULED",
  ...
  const openIncidents = incidents.map((i: Record<string, unknown>) => ({
    ...
    vehiclePlate:
      (
        vehicles.find(
          (v: { id: unknown; reg_number: unknown }) => v.id === i.vehicle_id,
        ) as { reg_number?: string } | undefined
      )?.reg_number ?? String(i.vehicle_id),
    ...
    .map(
      (v: {
        ...
        const lastFill = meta.last_fuel_date
          ? Math.floor(
              (Date.now() - new Date(meta.last_fuel_date as string).getTime()) /
                86_400_000,
            )
          : null
        return {
          ...
          plate: v.reg_number,
          fuelPct,
          lastFillDaysAgo: lastFill,
        }
      },
    )
    .filter(
      (f: { fuelPct: number | null }) => f.fuelPct !== null && f.fuelPct < 30,
    )
    .sort(
      (a: { fuelPct: number | null }, b: { fuelPct: number | null }) =>
        (a.fuelPct ?? 100) - (b.fuelPct ?? 100),
    )
    .slice(0, 6) as {
    vehicleId: string
    plate: string
    fuelPct: number
    lastFillDaysAgo: number | null
  }[]
