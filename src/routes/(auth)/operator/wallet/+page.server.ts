...
export const load: PageServerLoad = async ({ locals }) => {
  ...
  if (!operatorActorCtx) throw redirect(303, '/app/dashboard?denied=operator_not_active')
...
export const actions: Actions = {
  withdraw: async ({ request, locals }) => {
    ...
    if (!operatorCtxData) return { error: 'No active operator account.', success: false }
    ...
    try {
      const response = await processMpesaPush(phone, amountKes, 'OP_WITHDRAW', 'Operator withdrawal')
      ...
      return { error: `Withdrawal failed: ${err instanceof Error ? err.message : 'Unknown error'}`, success: false }
    }
  },
