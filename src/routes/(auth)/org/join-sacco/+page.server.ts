...
export const actions: Actions = {
  join: async ({ request, locals }) => {
    ...
    if (!session?.user?.id) {
      return fail(401, { error: 'You must be logged in to request membership.' })
    }
    ...
    if (org.status === 'active')            return fail(409, { error: 'This SACCO already has an active chair.' })
    if (org.status !== 'pending_activation') return fail(409, { error: 'This SACCO is not currently accepting requests.' })
    ...
    if (existing?.length) {
      return fail(409, { error: `You already have a pending request for ${org.name}.` })
    }
    ...
    if (existingActors?.length) {
      ...
      const { data: newActor, error: actorErr } = await supabase
        .from('actors')
        .insert({ profile_id: session.user.id, type: 'GUEST', status: 'unverified' })
        .select('id')
        .single()

      if (actorErr || !newActor) return fail(500, { error: 'Could not create actor record.' })
      actorId = newActor.id
    }
    ...
    if (reqErr) return fail(500, { error: 'Could not submit your request. Please try again.' })

    await supabase.from('audit_logs').insert({
      event_type: 'org_join_requested',
      actor_id:   actorId,
      profile_id: session.user.id,
      details:    { organization_id: org_id, org_name: org.name, role: 'ORG_CHAIR' },
    }).then(({ error: e }) => { if (e) console.warn('[join-sacco] audit non-fatal:', e) })
