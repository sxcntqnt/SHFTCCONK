...
export const load: PageServerLoad = async ({ params, locals }) => {
  ...
  const actorCtx = userState.activeContexts.find(ctx => {
    ...
    if ([ACTOR_TYPES.SUPER_ADMIN, ACTOR_TYPES.ADMIN, ACTOR_TYPES.ORG_CHAIR].includes(
      ctx.type as any
    )) {
  ...
  if (!actorCtx) throw redirect(303, `/org/${orgId}/dashboard?denied=finance.list`)

  const isChair = [
    ACTOR_TYPES.SUPER_ADMIN, ACTOR_TYPES.ADMIN, ACTOR_TYPES.ORG_CHAIR,
  ].includes(actorCtx.type as any)

  // Org name from memberships
  const orgMembership = actorCtx.orgMemberships.find(m => m.organization_id === orgId)
  ...
  const levyTransactions: WalletTransaction[] = (reconResult.data ?? []).map(r => {
  ...
  const settlementTx: WalletTransaction[] = (settlementResult.data ?? []).map(r => ({
    id:          r.id,
  ...
  const vehicleCount = new Set((reconResult.data ?? []).map(r => r.vehicle_id)).size
...
export const actions: Actions = {
  settle: async ({ request, params, locals }) => {
    ...
    if (!canSettle) return { error: 'You do not have permission to initiate settlements.', success: false }
    ...
    try {
      ...
      const initiatingCtx = userState.activeContexts.find(ctx =>
        ctx.status === 'active' &&
        ctx.jurisdictions.some(j => j.scope_id === orgId || j.level === 'federal')
      )
      ...
      return { error: `Settlement failed: ${err instanceof Error ? err.message : 'Unknown error'}`, success: false }
    }
  },
}