...
export const load: PageServerLoad = async (event) => {
  ...
  const { data: row, error: vehicleErr } = await supabase
    .from("vehicles")
    .select(
      "id, reg_number, organization_id, route, status, active, capacity, owner_id, gps_lat, gps_lng, metadata",
    )
    .eq("id", vehicleId)
  ...
  const [complianceRes, reconciliationRes, ledgerRes] =
    await Promise.allSettled([
      ...
      supabase
        ...
        .select(
          "total_collected, expected_amount, variance, status, reconciled_date",
        )
        .eq("vehicle_id", vehicleId)
        .eq("organization_id", orgId)
  ...
  const totalCollected = recRows.reduce(
    (sum: number, r: { total_collected: number }) =>
      sum + Number(r.total_collected ?? 0),
    0,
  )
  const totalExpected = recRows.reduce(
    (sum: number, r: { expected_amount: number }) =>
      sum + Number(r.expected_amount ?? 0),
    0,
  )
  const trendPoints: number[] = recRows.map((r: { total_collected: number }) =>
