<script lang="ts">
  ...
  $: flaggedVehicles = alerts
    .filter(a => a.status === 'EXPIRED' || a.status === 'WARNING' || a.status === 'MEDIUM' || a.status === 'HIGH')
    .map(a => ({
      vehicle: a.vehicle,
      type: a.type,
      status: a.status,
  ...
  // Global compliance trend (optional analytics)
  const globalTrend = writable([]);
  ...
  onMount(() => {
    ...
    flaggedVehicles.forEach(v => {
      ...
      const color = v.status === 'EXPIRED' ? 'red' : v.status === 'WARNING' ? 'orange' : 'blue';
      const marker = L.circleMarker([v.gpsLat, v.gpsLng], {
        color,
        radius: 8,
        fillOpacity: 0.8
      }).addTo(mapInstance);
      marker.bindPopup(`<strong>${v.vehicle}</strong><br>Status: ${v.status}<br>Type: ${v.type}`);
    });
  });
</script>
...
<GlassCard class="mb-6">
  ...
  <p class="text-gray-600 mb-4">Global view of compliance alerts across all vehicles</p>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
  ...
  <GlassCard>
    ...
    <p class="text-2xl font-bold">{alerts.filter(a => a.status === 'EXPIRED' || a.status === 'WARNING').length}</p>
  </GlassCard>
  <GlassCard>
    <h4 class="font-semibold mb-1">Vehicles Monitored</h4>
    <p class="text-2xl font-bold">{[...new Set(alerts.map(a => a.vehicle))].length}</p>
  </GlassCard>
</div>
...
<GlassCard class="mb-6">
  ...
  <p class="text-gray-600 mb-2">Vehicles with warnings or expired compliance items</p>
  ...
  {#each flaggedVehicles as v}
    <GlassCard>
      <div class="flex justify-between items-center mb-2">
        ...
        <span class="text-gray-600">{alerts.filter(a => a.vehicle === v.vehicle).length} alerts</span>
      </div>
