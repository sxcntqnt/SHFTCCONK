<script lang="ts">
  ...
  function linkDevice() {
    if (!plateNumber.trim().match(/^[A-Z]{3}\s?\d{3}[A-Z]$/i)) {
      // Rough Kenyan plate regex
...
<div
  class="min-h-screen bg-gradient-to-br from-[#F5F5F7] to-[#E5E5EA] p-6 flex items-center justify-center font-sans"
>
  <div class="w-full max-w-md">
    <div
      class="card bg-white/90 backdrop-blur-2xl border border-white/60 shadow-2xl rounded-3xl overflow-hidden"
    >
        ...
        {#if step === 1}
          <div class="text-center space-y-6">
            <div class="relative inline-block mx-auto">
              <div
                class="w-28 h-28 bg-blue-100 rounded-full flex items-center justify-center"
              >
                <Radio
                  class="text-blue-600 {isScanning ? 'animate-pulse' : ''}"
                  size={56}
                />
              </div>
              {#if isScanning}
                <div
                  class="absolute inset-0 border-4 border-blue-300 rounded-full animate-ping"
                ></div>
              {/if}
            </div>
            ...
            <p class="text-gray-600">
              Securely link tracker to matatu — anti-spoofing &
              tamper-protected.
            </p>
            ...
            <div
              class="bg-green-50 p-4 rounded-2xl text-sm text-green-800 flex items-start gap-3"
            >
          ...
          <div class="space-y-6">
            <header class="text-center">
              ...
              <p class="text-gray-600 mt-1">
                ID: <span class="font-mono font-bold">{deviceId}</span>
              </p>
              {#if qrScanned}
                <p class="text-xs text-indigo-600 mt-1">
                  Secure QR attestation verified
                </p>
              {/if}
            </header>
            ...
            <div>
              <label
                class="block uppercase text-xs font-bold text-gray-600 mb-2 tracking-wider"
                >Matatu Plate Number *</label
              >
              <input
            ...
            <div class="bg-amber-50 p-5 rounded-2xl text-sm space-y-3">
              <div class="flex items-start gap-3">
                ...
                <p>
                  <strong>Important:</strong> Linking enables real-time tracking.
                  Owner owns detailed data; SACCO sees only compliance views.
                </p>
              </div>
              <div class="flex items-start gap-3">
            ...
            <div class="flex gap-4">
              <button
                on:click={() => (step = 1)}
                class="btn btn-ghost flex-1 rounded-full h-14">Back</button
              >
              <button
                on:click={linkDevice}
        ...
        {:else}
          <div class="text-center py-10 space-y-6">
            ...
            <p class="text-gray-700 px-4">
              Tracker <span class="font-mono font-bold">{deviceId}</span> is now
              secured to <span class="font-bold uppercase">{plateNumber}</span>.
            </p>
            <p class="text-sm text-gray-600">
              Real-time telemetry active • Tamper monitoring enabled • MQTT/TLS
              secured
            </p>
