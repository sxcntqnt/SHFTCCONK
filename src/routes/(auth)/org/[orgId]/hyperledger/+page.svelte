<script lang="ts">
  ...
  const queueItems: QueueItem[] = Array.isArray(enrollmentQueue)
    ? enrollmentQueue
    : []
  ...
  const exhaustedCount = queueItems.filter(
...
<div class="min-h-screen bg-gray-50 p-6 lg:p-8">
  ...
  <div class="mb-8 flex items-start justify-between">
    ...
    {#if exhaustedCount > 0}
      <div
        class="flex items-center gap-2 rounded-lg border border-rose-200 bg-rose-50 px-4 py-2.5 text-sm text-rose-700"
      >
      ...
      {#each tabs as tab}
        ...
        >
          ...
          {#if tab.id === "queue" && exhaustedCount > 0}
            <span
              class="ml-1 rounded-full bg-rose-100 px-1.5 py-0.5 text-xs font-bold text-rose-600"
            >
              {exhaustedCount}
            </span>
          {/if}
        ...
        {:else}
          <div class="mb-4 flex items-center justify-between">
            ...
            {#if exhaustedCount > 0}
              <p class="text-xs text-rose-600 font-medium">
                {exhaustedCount} exhausted — re-trigger from admin panel or wait for
                manual review
              </p>
            {/if}
          </div>
          ...
          <div class="space-y-3">
            {#each queueItems as item}
              <div
                class={`rounded-lg border p-4 ${queueStatusClass(item.status)}`}
              >
          ...
          <p class="mt-4 text-xs text-gray-400">
            Pending and retrying items are processed automatically every 2
            minutes. Exhausted items require a manual re-trigger.
          </p>
        {/if}
      </div>
    {/if}
