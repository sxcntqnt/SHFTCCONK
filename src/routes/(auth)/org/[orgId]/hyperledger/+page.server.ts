...
export const load: PageServerLoad = async ({ locals, params }) => {
  ...
  const isChair = userState.activeContexts.some(ctx => {
    ...
    if ([ACTOR_TYPES.SUPER_ADMIN, ACTOR_TYPES.ADMIN].includes(
  ...
  const isStaff = !isChair && userState.activeContexts.some(ctx => {
    if (ctx.status !== 'active')             return false
  ...
  if (isChair) {
    ...
    const jurisdictionActorIds = userState.activeContexts
      .filter(ctx =>
        ctx.jurisdictions.some(j => j.level === 'org' && j.scope_id === orgId)
      )
      .map(ctx => ctx.actorId)

    const allOrgActorIds = [...new Set([...orgActorIds, ...jurisdictionActorIds])]

    if (allOrgActorIds.length > 0) {
      // Fetch queue items for all actors in this org — show non-success items
      const { data: queueItems } = await supabase
        .from('hyperledger_enrollment_queue')
        .select('id, actor_id, intent, event_name, status, attempts, last_error, created_at')
