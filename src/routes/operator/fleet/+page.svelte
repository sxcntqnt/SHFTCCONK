<script lang="ts">
  ...
  } from "$lib/features/fleet/stores/fleet.store"
  import { complianceStore } from "$lib/features/fleet/stores/compliance.store"
  import { financeStore } from "$lib/features/fleet/stores/finance.store"
...
{#if vehicle}
        ...
        <ul class="space-y-2">
          {#each alerts as alert}
            <li class="bg-red-100 text-red-800 rounded-md px-3 py-2">
              <strong>{alert.type}</strong> - Expires: {alert.expiryDate} - Status:
              {alert.status}
            </li>
          {/each}
        </ul>
  ...
  <GlassCard>
    ...
    <p class="text-gray-700">
      Total Revenue Collected: <strong>KES {revenue.toLocaleString()}</strong>
    </p>
  </GlassCard>
{:else}
