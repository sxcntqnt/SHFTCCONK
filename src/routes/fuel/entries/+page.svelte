<script lang="ts">
  ...
  let { data }: { data: PageData } = $props();  const { monthlyData } = data;
...
<div class="w-full max-w-6xl space-y-12 pb-16">
  <div class="text-center">
    ...
    <p class="opacity-70 text-white">Monthly overview — consumption, cost and efficiency</p>
  </div>
  <!-- Filter controls -->
<div class="flex flex-wrap justify-center gap-6 mb-8">
  ...
  <button
    ...
    on:click={() => { fromMonth = availableMonths[0]; toMonth = availableMonths.at(-1); }}
  >
    Reset
  </button>
</div>
  ...
  <div class="card p-6 md:p-8">
    <h3 class="text-xl font-semibold mb-6 text-center text-white">Monthly Fuel Consumption (Liters)</h3>
    <div class="h-80">
      <Chart
        data={chartData}
        x="label"
      ...
      >
        <Svg>
          <Axis placement="left" label="Liters" grid rule ticks={6} labelStyle={{ fill: '#f1f5f9', fontWeight: 500 }} />
          <Axis placement="bottom" label="Month" rule labelStyle={{ fill: '#f1f5f9', fontWeight: 500 }} />
          <Bars radius={8} rounded="top" fill="#3b82f6" stroke="#1e40af" strokeWidth={1.5} />
          <Highlight area bar={{ radius: 10, fill: "#60a5fa", opacity: 0.85 }} />
          <Rule y={avgLiters} class="stroke-red-400 stroke-2 [stroke-dasharray:6_4] [stroke-linecap:round]" />
        </Svg>
  ...
  <div class="card p-6 md:p-8">
    <h3 class="text-xl font-semibold mb-6 text-center text-white">Fleet Efficiency Trend (km/L)</h3>
    <div class="h-80">
      <Chart
        data={chartData}
        x="label"
      ...
      >
        <Svg>
          <Axis placement="left" label="km / L" grid rule ticks={6} labelStyle={{ fill: '#f1f5f9', fontWeight: 500 }} />
          <Axis placement="bottom" label="Month" rule labelStyle={{ fill: '#f1f5f9', fontWeight: 500 }} />
  ...
  <div class="card p-6 md:p-8">
    <h3 class="text-xl font-semibold mb-6 text-center text-white">Monthly Fuel Cost (KSh)</h3>
    <div class="h-80">
      <Chart
        data={chartData}
        x="label"
      ...
      >
        <Svg>
          <Axis placement="left" label="Cost (KSh)" grid rule ticks={6} labelStyle={{ fill: '#f1f5f9', fontWeight: 500 }} />
          <Axis placement="bottom" label="Month" rule labelStyle={{ fill: '#f1f5f9', fontWeight: 500 }} />
          <Bars radius={6} fill="#facc15" stroke="#b45309" strokeWidth={1.5} />
          <Highlight area bar={{ radius: 8, fill: "rgba(250,204,21,0.5)", opacity: 0.9 }} />
          <Rule y={avgCost} class="stroke-yellow-400 stroke-2 [stroke-dasharray:6_4] [stroke-linecap:round]" />
        </Svg>
