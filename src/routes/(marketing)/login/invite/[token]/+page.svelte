<script lang="ts">
  ...
  async function redeemInvite() {
    ...
    try {
      ...
      const { data: rpcData, error: rpcErr } = await data.supabase.rpc(
        "redeem_invite",
        {
          p_token: data.token,
          p_password: password,
      ...
      // Track invite redemption — sacco staff onboarding conversion
      if (browser && payload?.profile_id) {
        posthog.identify(payload.profile_id, {
          email: data.invite.email,
          role: payload.actor_type ?? data.invite.role,
          organization: data.invite.organizationName,
        })
  ...
  const roleLabel = $derived(
    ROLE_LABELS[data.invite?.role ?? ""] ?? data.invite?.role ?? "Unknown",
  )
...
<div class="redeem-wrap">
  ...
  {#if step === "invalid"}
    <div class="invalid-card">
      ...
      <p class="invalid-desc">
        {error ??
          "This invitation link has expired, already been used, or the code is incorrect."}
        Invitation links expire after 72 hours for security.
      </p>
  ...
  {:else if step === "confirm"}
    <div class="page-header">
      ...
      <p class="page-sub">
        Review your pre-assigned role and organisation before setting your
        password.
      </p>
    </div>
    ...
    <div class="identity-card">
      {#each [{ label: "Email", value: data.invite.email }, { label: "Role", value: null, pill: roleLabel }, { label: "Org", value: data.invite.organizationName }, { label: "Set by", value: data.invite.invitedBy }, { label: "Expires", value: null, expires: data.invite.expiresAt }] as row}
    ...
    <div class="lock-notice">
      ...
      <p>
        Your role and organisation cannot be changed through this form. If these
        details are incorrect, contact <strong>your sacco administrator</strong>
        or <strong>support@matatupulse.co.ke</strong> before proceeding.
      </p>
    </div>
  ...
  {:else if step === "set-password"}
    <div class="page-header">
      ...
      <p class="page-sub">
        Choose a strong password for your <span
          style="color:var(--orange);font-weight:600;">{roleLabel}</span
        >
        account at {data.invite.organizationName}.
      </p>
    </div>
    ...
    <div class="field">
      ...
      <div class="strength-bar">
        {#each Array(5) as _, i}
          <div
            class="strength-seg {i < strength ? `active-${strength}` : ''}"
          ></div>
        {/each}
      </div>
      <!-- Checks -->
      <div class="checks">
        {#each [{ key: "length", label: "At least 8 characters" }, { key: "upper", label: "One uppercase letter" }, { key: "lower", label: "One lowercase letter" }, { key: "digit", label: "One number" }] as c}
          <div
            class="check {checks[c.key as keyof typeof checks] ? 'pass' : ''}"
          >
            <div class="check-dot">
    ...
    <div class="field">
      ...
      {#if confirmPassword && !checks.match}
        <p style="font-size:0.75rem;color:#f87171;margin-top:5px;">
    ...
    <div class="success-card">
      ...
      <p class="success-sub">
        ...
        <strong style="color:var(--text-1)"
          >{data.invite.organizationName}</strong
        >
...
<style>
  ...
  .strength-seg.active-1 {
    background: #f87171;
  }
  .strength-seg.active-2 {
    background: #fb923c;
  }
  .strength-seg.active-3 {
    background: #facc15;
  }
  .strength-seg.active-4 {
    background: #4ade80;
  }
  .strength-seg.active-5 {
    background: var(--teal);
  }
