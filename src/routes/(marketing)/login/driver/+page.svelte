<script lang="ts">
  ...
  async function requestOtp() {
    error = null
    if (!phone.trim()) { error = "Please enter your phone number."; return }
    loading = true
    try {
  ...
  async function verifyOtp() {
    error = null
    if (otp.trim().length < 6) { error = "Please enter the 6-digit code."; return }
    loading = true
    try {
      const normalised = normalisePhone(phone)
      ...
      const { data: rpcData, error: rpcErr } = await data.supabase.rpc("bootstrap_session")
      if (rpcErr) { console.error("bootstrap_session", rpcErr); goto("/account"); return }
  ...
  onMount(() => () => { if (countdownTimer) clearInterval(countdownTimer) })
</script>
...
<style>
  ...
  /* Role badge */
  .role-badges { display: flex; gap: 6px; margin-bottom: 24px; flex-wrap: wrap; }
  .role-badge {
    padding: 3px 10px; border-radius: 100px;
    font-size: 0.65rem; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase;
    background: rgba(242,101,34,0.1); border: 1px solid rgba(242,101,34,0.22); color: var(--orange);
  }

  /* Phone step */
  .phone-prefix-row { display: flex; gap: 8px; margin-bottom: 16px; }
  .prefix-badge {
    display: flex; align-items: center; padding: 11px 14px;
    background: var(--ink-2); border: 1px solid var(--rim-2); border-radius: 10px;
    font-size: 0.875rem; color: var(--text-2); font-weight: 600; flex-shrink: 0;
    gap: 6px;
  }
  ...
  input.otp-input {
    ...
    font-size: 1.4rem; font-family: var(--font-display); font-weight: 800;
    padding: 14px;
  }
  ...
  .btn-submit:hover:not(:disabled) { background: #d95618; transform: translateY(-1px); box-shadow: 0 8px 32px rgba(242,101,34,0.42); }
  .btn-submit:disabled { opacity: 0.55; cursor: not-allowed; transform: none; }

  /* OTP boxes visual */
  .otp-display {
    display: flex; gap: 6px; justify-content: center; margin-bottom: 4px; pointer-events: none;
    position: absolute; top: 50%; left: 50%; transform: translate(-50%,-50%);
    width: 100%;
  }
  .otp-box {
    width: 36px; height: 44px; border: 1.5px solid var(--rim-2);
    border-radius: 8px; display: flex; align-items: center; justify-content: center;
    font-family: var(--font-display); font-size: 1.2rem; font-weight: 800;
    color: var(--text-1);
    transition: border-color 0.2s;
  }
  .otp-box.filled { border-color: var(--orange); background: rgba(242,101,34,0.06); }
  .otp-input-wrap { position: relative; height: 56px; margin-bottom: 12px; }
  .otp-input-wrap input { position: absolute; inset: 0; opacity: 0; height: 100%; cursor: text; }
  ...
  .info-box {
    ...
    background: var(--surface); border: 1px solid var(--rim-2); border-radius: 10px;
  }
  .info-box svg { color: var(--text-3); flex-shrink: 0; margin-top: 1px; }
  .info-box p { font-size: 0.82rem; color: var(--text-2); line-height: 1.55; }
  ...
  .resend-countdown { font-size: 0.78rem; color: var(--text-3); }
...
<div class="driver-wrap">
  ...
  <div class="page-header">
    ...
    <h1 class="page-title">{step === "otp" ? "Enter Your Code" : "Driver & Conductor Sign In"}</h1>
    <p class="page-sub">
      {step === "otp"
  ...
  {#if step === "phone"}
    <div>
      ...
      <p class="input-hint">Enter your Safaricom, Airtel, or Telkom number. We'll send you a one-time code.</p>
  ...
  {:else if step === "otp"}
    <div class="info-box">
      ...
      <p>Code sent to <strong>{phone}</strong>. <button style="background:none;border:none;color:var(--orange);font-size:inherit;font-weight:600;cursor:pointer;padding:0;" onclick={() => { step = "phone"; error = null; }}>Change number</button></p>
    </div>
    ...
    <p class="input-hint">The code expires in 10 minutes. Auto-submits when all 6 digits are entered.</p>

    <button class="btn-submit" onclick={verifyOtp} disabled={loading || otp.length < 6}>
      {#if loading}<div class="spinner"></div>{:else}
        Verify & Sign In
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
      {/if}
  ...
  <div class="auth-footer" style="margin-top: 24px;">
    <div class="auth-footer-row">Not a driver? <a href="/login">Back to all sign-in options</a></div>
    <div class="auth-footer-row">Need help? <a href="tel:+254700000000">Call our ops team</a></div>
  </div>
