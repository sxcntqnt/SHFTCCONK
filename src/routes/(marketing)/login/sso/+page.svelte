<script lang="ts">
  ...
  async function signInWithSSO(provId: string, hd: string | null, provider: string) {
    error = null
    loading = provId
    try {
...
<style>
  ...
  /* Role pills */
  .role-pills { display: flex; flex-wrap: wrap; gap: 5px; margin-bottom: 22px; }
  .role-pill {
  ...
  .security-banner svg { color: var(--orange); flex-shrink: 0; margin-top: 1px; }
  .security-banner p { font-size: 0.78rem; color: var(--text-2); line-height: 1.55; }
  .security-banner strong { color: var(--text-1); }

  /* Provider list */
  .provider-list { display: flex; flex-direction: column; gap: 8px; margin-bottom: 22px; }
  .provider-btn {
  ...
  .provider-btn:hover:not(:disabled) { border-color: rgba(242,101,34,0.3); transform: translateX(3px); }
  .provider-btn:disabled { opacity: 0.6; cursor: not-allowed; }
  .provider-logo {
  ...
  .provider-name { font-family: var(--font-display); font-size: 0.875rem; font-weight: 700; color: var(--text-1); margin-bottom: 1px; }
  .provider-full { font-size: 0.72rem; color: var(--text-3); }
  .provider-domain {
    font-size: 0.65rem; font-weight: 600; letter-spacing: 0.06em;
    padding: 2px 7px; border-radius: 100px;
    background: rgba(0,176,155,0.1); color: var(--teal); border: 1px solid rgba(0,176,155,0.22);
    flex-shrink: 0;
  }
  .provider-arrow { color: var(--text-3); flex-shrink: 0; transition: color 0.2s; }
  .provider-btn:hover .provider-arrow { color: var(--orange); }

  @keyframes spin { to { transform: rotate(360deg); } }
  .spinner { width: 14px; height: 14px; border: 2px solid rgba(242,101,34,0.3); border-top-color: var(--orange); border-radius: 50%; animation: spin 0.7s linear infinite; }

  /* Approval note */
  .approval-note {
    display: flex; align-items: flex-start; gap: 10px;
    padding: 12px 14px; margin-bottom: 20px;
    background: var(--surface); border: 1px solid var(--rim); border-radius: 10px;
  }
  .approval-note svg { color: var(--text-3); flex-shrink: 0; margin-top: 1px; }
  .approval-note p { font-size: 0.78rem; color: var(--text-3); line-height: 1.55; }
  .approval-note strong { color: var(--text-2); }
...
<div class="sso-wrap">
  ...
  <div class="page-header">
    ...
    <p class="page-sub">Authenticate with your government-issued credentials. We never store your institutional password.</p>
  </div>
  ...
  <div class="security-banner">
    ...
    <p><strong>Federated authentication only.</strong> You authenticate against your own organisation's identity provider. Matatu Pulse never sees your institutional credentials. Access is domain-restricted and verified against a pre-approved list.</p>
  </div>
  ...
  <div class="approval-note">
    ...
    <p>First-time access requires approval from Matatu Pulse. SSO sign-in will succeed, but <strong>dashboard access is enabled within one business day</strong> of your initial sign-in. Contact <a href="mailto:gov@matatupulse.co.ke" style="color:var(--orange);font-weight:600;">gov@matatupulse.co.ke</a> to expedite.</p>
  </div>

  <div class="auth-footer">
    <div class="auth-footer-row">Your organisation isn't listed? <a href="/contact?type=regulatory">Request access</a></div>
    <div class="auth-footer-row">Not a regulator? <a href="/login">Back to all sign-in options</a></div>
  </div>
