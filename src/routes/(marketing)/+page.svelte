<script lang="ts">
  ...
  export const PLATFORM_CAPABILITIES: readonly PlatformCapability[] = [
    {
      ...
      description: "Continuous GPS streaming from installed trackers provides second-level vehicle position accuracy.",
      icon: "tracking",
    ...
    },
    {
      ...
      description: "Routes adapt using congestion patterns and historical trip duration modeling.",
      icon: "routes",
    ...
    },
    {
      ...
      description: "Arrival prediction, delays, diversions, and approaching vehicle notifications.",
      icon: "notifications",
    ...
    },
    {
      ...
      description: "Trip cycles, idle time detection, load balancing, and revenue opportunity identification.",
      icon: "analytics",
  ...
  export const COMMUTER_WORKFLOWS: readonly CommuterWorkflow[] = [
    {
      ...
      description: "Select a route and instantly see nearby vehicles approaching your stage.",
      link: "/app/dashboard",
    },
    {
      icon: "routes",
  ...
  export const TESTIMONIALS: readonly Testimonial[] = [
    { name: "John Mwangi",    userType: "Commuter",     testimony: "I no longer wait blindly at the stage. I leave only when the matatu is close.",       rating: 5 },
    { name: "Grace Wanjiku",  userType: "Operator",     testimony: "We can now see delays immediately and redirect vehicles accordingly.",                  rating: 4 },
    { name: "Peter Kamau",    userType: "Commuter",     testimony: "Arrival prediction completely changed my morning routine.",                             rating: 5 },
    { name: "Mary Achieng",   userType: "Commuter",     testimony: "Notifications help me plan transfers without stress.",                                  rating: 4 },
    { name: "David Otieno",   userType: "Fleet Owner",  testimony: "We identified idle vehicles and increased daily trips.",                                rating: 5 },
    { name: "Sarah Njeri",    userType: "Commuter",     testimony: "Fastest route suggestions save me a lot of time every week.",                          rating: 4 },
    { name: "James Njoroge",  userType: "Commuter",     testimony: "The system makes commuting predictable for the first time.",                           rating: 5 },
    { name: "Esther Muthoni", userType: "Operator",     testimony: "Passengers complain less because they already know delays.",                           rating: 4 },
    { name: "Michael Omondi", userType: "Commuter",     testimony: "Finally public transport behaves like a modern system.",                               rating: 5 },
  ]

  const featuredTestimonials = TESTIMONIALS.filter(t => t.rating === 5).slice(0, 5)
  ...
  const commuterFeatures     = platformCapabilities.filter(f => f.audience.includes("Commuter"))
...
<svelte:head>
  ...
  <meta name="description" content="Live matatu tracking in Nairobi. See real-time positions, accurate ETAs and get 2–3 min arrival alerts. Free for commuters." />
  {@html jsonldScript}
</svelte:head>
...
<style>
  ...
  .hero-overlay {
    position: absolute; inset: 0;
    background:
      linear-gradient(to bottom, rgba(10,10,12,0.35) 0%, rgba(10,10,12,0.65) 60%, rgba(10,10,12,1) 100%),
  ...
  .btn-hero-primary:hover { background: #d95618; transform: translateY(-2px); box-shadow: 0 12px 40px rgba(242,101,34,0.5); }
  .btn-hero-secondary {
    display: inline-flex; align-items: center; gap: 10px;
    padding: 14px 28px;
  ...
  .btn-hero-secondary:hover { background: rgba(255,255,255,0.12); border-color: rgba(255,255,255,0.3); transform: translateY(-2px); }
  .hero-note {
    font-size: 0.8rem; color: var(--text-3); letter-spacing: 0.02em;
  }
  ...
  .partners-logos {
    ...
    gap: 24px 48px; align-items: center; justify-content: center;
  }
  .partner-logo {
    height: 36px; opacity: 0.4;
    filter: brightness(0) invert(1);
    transition: opacity 0.3s, filter 0.3s;
  }
  .partner-logo:hover { opacity: 0.85; filter: brightness(1) invert(0); }
  ...
  .steps-grid::before {
    content: '';
    position: absolute; top: 52px; left: calc(16.6% + 1px); right: calc(16.6% + 1px);
    height: 1px;
  ...
  .step-icon-ring img { width: 28px; height: 28px; object-fit: contain; }
  .step-title {
    font-family: var(--font-display);
  ...
  /* ── FEATURES ── */
  .features { padding: 100px 2rem; background: var(--ink-2); }
  ...
  .feature-card:hover { border-color: rgba(242,101,34,0.3); transform: translateY(-4px); }
  .feature-icon {
  ...
  .audience-tag {
    ...
    text-transform: uppercase;
    background: var(--rim); color: var(--text-3);
    border: 1px solid var(--rim-2);
  }
  ...
  .testimonial-card:hover { border-color: var(--rim-2); }
  .testimonial-card.featured {
    border-color: rgba(242,101,34,0.25);
    background: rgba(242,101,34,0.04);
  }
  .stars { display: flex; gap: 3px; margin-bottom: 16px; }
  .star { font-size: 0.85rem; color: var(--orange); }
  .star.empty { color: var(--rim-2); }
  .testimonial-text {
  ...
  .operator-benefits { list-style: none; display: flex; flex-direction: column; gap: 14px; margin-bottom: 40px; }
  .operator-benefit {
  ...
  .benefit-check {
    ...
    background: rgba(0,176,155,0.12); border: 1px solid rgba(0,176,155,0.3);
    display: flex; align-items: center; justify-content: center;
    margin-top: 1px;
  }
  .benefit-check svg { color: var(--teal); }
  .btn-teal {
    display: inline-flex; align-items: center; gap: 8px;
    padding: 13px 28px;
  ...
  .btn-teal:hover { background: rgba(0,176,155,0.1); border-color: var(--teal); transform: translateY(-1px); }
  .operator-visual {
    background: var(--surface);
    border: 1px solid var(--rim);
  ...
  .op-stat-val {
    ...
    color: var(--teal); letter-spacing: -0.03em; margin-bottom: 4px;
  }
  .op-stat-lbl { font-size: 0.78rem; color: var(--text-3); font-weight: 500; }
  ...
  .final-cta::before {
    ...
    position: absolute; inset: 0;
    background:
      radial-gradient(ellipse 70% 60% at 50% 0%, rgba(242,101,34,0.14), transparent 65%);
    pointer-events: none;
  }
  ...
  .final-note { font-size: 0.8rem; color: var(--text-3); }

  /* ── RESPONSIVE ── */
  @media (max-width: 1024px) {
    .steps-grid { grid-template-columns: 1fr; gap: 0; }
    .steps-grid::before { display: none; }
    .step-card { padding: 28px 24px; border-bottom: 1px solid var(--rim); }
  ...
  @media (max-width: 640px) {
    ...
    .stat-item:nth-child(3) { border-right: 1px solid var(--rim); }
...
<section class="hero">
  ...
  <div class="hero-inner">
    ...
    <p class="hero-note">Already helping thousands of Nairobi commuters save time every day</p>
  </div>
</section>
...
<section class="how-it-works">
  <div class="how-inner">
    <div class="section-header">
      ...
      <p class="section-sub">Three simple steps from uncertainty to a predictable, stress-free commute.</p>
    </div>
...
<section class="features">
  <div class="features-inner">
    <div class="section-header">
      ...
      <p class="section-sub">Everything you need to reclaim your time — from live tracking to smart route comparison.</p>
    </div>
...
<section class="testimonials">
  <div class="testimonials-inner">
    <div class="section-header">
      ...
      <p class="section-sub">From daily riders to fleet managers — hear what the people using it every day have to say.</p>
    </div>
...
<section class="operator-callout">
  <div class="operator-inner">
    <div class="operator-content">
      ...
      <p>
        Get real-time fleet intelligence — track every vehicle, spot delays before
        passengers do, and unlock revenue insights you didn't know were hiding in your routes.
      </p>
    ...
    <div class="operator-visual">
      <div class="op-stat-grid">
        {#each [
          { val: "18%",  lbl: "More Trips Per Day" },
          { val: "34%",  lbl: "Reduced Idle Time" },
          { val: "2.4×", lbl: "Faster Incident Response" },
          { val: "91%",  lbl: "Operator Satisfaction" },
        ] as s}
          <div class="op-stat">
...
<section class="final-cta">
  <div class="final-cta-inner">
    ...
    <p>Download the app and take back control of your time. Live tracking, arrival alerts, and route optimization — all free for riders.</p>
    ...
    <p class="final-note">No account needed to start tracking. Instant access.</p>
  </div>
</section>