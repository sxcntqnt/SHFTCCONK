<script lang="ts">
  ...
  let {
    alerts = $bindable([]), // or just alerts = [] if you don't need two-way binding
  }: { alerts?: Alert[] } = $props()

  // Enrich severity display
  function sev(a: Alert) {
    if (a.severity === "delay")
      return { label: "DELAY", cls: "delay", icon: "⚠" }
    if (a.severity === "early")
      return { label: "ARRIVING", cls: "early", icon: "●" }
...
<div class="alerts-wrap">
  ...
  <div class="alerts-footer">
    <span class="footer-note"
      >Alerts fire at ≤ 3 min ETA · updated every 5s</span
    >
  </div>
</div>

<style>
  /* Your original styles are unchanged – they work perfectly in Svelte 5 */
  .alerts-wrap {
    width: 100%;
    height: 100%;
  ...
  .fill-early {
    background: linear-gradient(90deg, rgba(0, 176, 155, 0.3), #00b09b);
  }
  .fill-delay {
    background: linear-gradient(90deg, rgba(242, 101, 34, 0.3), #f26522);
  }
  .fill-info {
    background: rgba(255, 255, 255, 0.2);
  }
