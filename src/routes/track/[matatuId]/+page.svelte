<script lang="ts">
  ...
  const K = 0.5;
  function kalmanUpdate(pos: { lat: number; lng: number }, estimate: { lat: number; lng: number }) {
    return {
  ...
  function animateAlongRoute(marker: FleetMarker, route: { lat: number; lng: number }[]) {
    ...
    gsap.to(marker.coords, {
      ...
      ease: 'power1.inOut',
      onUpdate: () => {
        markers.update(list => list.map(m => (m.id === marker.id ? { ...m } : m)));
      },
      onComplete: () => {
  ...
  let mapCenter = reservedMatatus[0]?.routePolyline[0] || { lat: 1.2921, lng: 36.8219 };
  let mapZoom = 15;
...
<div class="w-screen h-screen bg-gradient-to-b from-gray-50 via-white to-gray-100 flex items-center justify-center overflow-hidden">
  {#if reservedMatatus.length > 0}
    <div class="w-full h-full flex flex-col items-center justify-center">
      <h1 class="text-4xl md:text-5xl font-semibold text-gray-900 tracking-tight mb-4">
        Live Tracking
      </h1>
      <p class="text-xl text-gray-600 mb-8 max-w-3xl">
        Your reserved matatus — watch them move in real time.
      </p>
      <div class="rounded-3xl overflow-hidden shadow-2xl ring-1 ring-gray-200/60 bg-white w-full h-[80%]">
        <MapView markers={$markers} initialCenter={mapCenter} initialZoom={mapZoom} />
      </div>
    </div>
  {:else}
    <div bind:this={raceContainer} class="relative flex flex-col items-center justify-center w-full h-full text-center">
      ...
      <div class="relative z-10 max-w-2xl text-center">
        <h1 class="text-4xl md:text-5xl lg:text-6xl font-extrabold text-gray-900 tracking-wide mb-6 leading-tight bg-clip-text text-transparent bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 drop-shadow-md">
  They're Leaving Without You
</h1>

        <p class="text-lg md:text-xl text-gray-700 font-medium italic mb-12 max-w-xl mx-auto leading-relaxed">
  Reserve your seat now and join the ride. <span class="font-semibold text-indigo-600">Watch your matatu live</span> as it comes for you.
</p>
