<script lang="ts">
  ...
  const MENU_CONFIG = [
    ...
    },
    {
      ...
      label: "Live Feed",
      icon: "🚌",
      roles: [ROLES.PASSENGER, ROLES.PLANNER, ROLES.REGULATOR],
    },
    {
      id: "telemetry",
      href: "/app/sync",
      ...
      icon: "📡",
      roles: [ROLES.ORGANIZATION, ROLES.ADMIN, ROLES.OWNER],
    },
    {
      id: "fleet",
      href: "/app/fleet",
      label: "Fleet Manager",
      icon: "📊",
      roles: [ROLES.OWNER, ROLES.ORGANIZATION],
    },
    {
      id: "reserve",
      href: "/reserve",
      label: "Reservations",
      icon: "🎫",
      roles: [ROLES.PASSENGER, ROLES.ADMIN],
    },
    {
      id: "track",
      href: "/track",
    ...
    },
    {
      ...
      label: "Race Control",
      icon: "🏁",
      roles: [ROLES.PASSENGER, ROLES.OWNER, ROLES.ORGANIZATION, ROLES.ADMIN],
    },
    {
      id: "chat",
      href: "/chat",
  ...
  const visibleMenu = derived(authStore, ($authStore) =>
    MENU_CONFIG.filter(
      (item) =>
        item.roles.includes("*") || item.roles.includes($authStore?.role ?? ""),
    ),
  )
...
<!-- Mobile Header -->
<div
  class="md:hidden flex items-center justify-between p-4 border-b bg-white shadow-sm sticky top-0 z-30"
>
  <h1 class="text-lg font-bold tracking-tighter">MATATU OS</h1>
  <button
    aria-label="Open menu"
    bind:this={menuButton}
    class="p-2 rounded-md bg-gray-50 border border-gray-200"
...
>
  ...
  <div class="flex items-center p-6 border-b border-gray-200 min-h-[100px]">
    <div
      class="flex flex-col gap-1.5 min-w-0 flex-1 transition-all duration-300"
    >
      <h1
        class="text-2xl font-black tracking-tighter text-[#0a0a0a] transition-all duration-300 origin-left"
      ...
      <div class="flex items-center gap-3">
        <div class="w-8 flex justify-center flex-shrink-0">
          <span
            class="w-2.5 h-2.5 rounded-full animate-pulse ring-2 {dotColorClass}"
            transition:fade
          ></span>
        </div>
        <span
          class="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 truncate transition-all duration-300 delay-75"
          class:opacity-0={collapsed && !canHoverExpand}
    ...
    >
      <svg
        class="w-4 h-4 text-gray-500 transition-transform duration-500"
    ...
    {#each $visibleMenu as item}
      <a
        ...
        class="flex items-center h-12 px-3 rounded-xl font-bold text-gray-600 transition-all duration-200 ease-out group/link
          ...
          : 'hover:bg-gray-200/40 hover:text-gray-900'}"
      >
        <span
          class="text-xl w-8 flex justify-center flex-shrink-0 transition-transform duration-200 group-hover/link:scale-110"
          >{item.icon}</span
        >
        <span
          class="ml-3 truncate text-sm transition-all duration-300"
          class:opacity-0={collapsed && !canHoverExpand}
  ...
  <div class="p-3 mt-auto border-t border-gray-200 bg-gray-50/50">
    ...
    >
      <div
        class="w-10 h-10 rounded-xl bg-gradient-to-tr from-blue-600 to-indigo-500 flex items-center justify-center text-white font-bold text-lg shadow-inner flex-shrink-0"
        transition:fade
      >
        {$authStore?.fullName?.[0] ?? "U"}
      </div>
      <div
        class="ml-3 min-w-0 transition-all duration-300 overflow-hidden"
        class:w-0={collapsed && !canHoverExpand}
        class:opacity-0={collapsed && !canHoverExpand}
        class:w-full={!collapsed || canHoverExpand}
      >
        <p class="text-sm font-bold truncate" transition:fade>
          {$authStore?.fullName ?? "Guest authStore"}
        </p>
        <p
          class="text-[10px] text-gray-400 font-bold uppercase truncate"
          transition:fade
        >
          {$authStore?.sacco ?? "Personal"}
        </p>
      </div>
    </div>
  </div>
...
{#if showDrawer}
  <div class="fixed inset-0 z-40 md:hidden">
    ...
    <div
      ...
      class="absolute left-0 top-0 bottom-0 w-80 bg-white shadow-2xl flex flex-col p-4 overflow-y-auto"
    >
      ...
      <div class="mt-6 p-3 border-t flex items-center gap-3">
        <div
          class="w-10 h-10 rounded-full bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center text-white font-bold"
          transition:fade
        >
          {$authStore?.fullName?.[0] ?? "U"}
        </div>
        <div>
          <div class="font-semibold" transition:fade>
            {$authStore?.fullName ?? "Guest authStore"}
          </div>
...
<style>
  ...
  aside.group:hover {
    width: 17rem;
    box-shadow:
      20px 0 25px -5px rgba(0, 0, 0, 0.05),
      10px 0 10px -5px rgba(0, 0, 0, 0.02);
  }
