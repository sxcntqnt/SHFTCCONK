<script lang="ts">
  ...
  function handleModelLoad(sceneArg: THREE.Group) {
    ...
    setTimeout(() => {
      ...
      if (box.isEmpty() || box.getSize(new THREE.Vector3()).length() < 0.1) {
        console.warn(
          "[SceneContents] Bounds still empty after 200ms — retrying in 500ms",
        )
  ...
  function placeCamera(sceneArg: THREE.Group) {
    ...
    busInterior.geometry.busBounds = new THREE.Box3().setFromObject(
      exteriorGroupRef,
    )
  ...
  onMount(async () => {
    ...
    new RGBELoader().load(
      ...
      (err) =>
        console.warn(
          "[SceneContents] HDR failed — check /static/hdr/garage.hdr:",
          err,
        ),
    )
  ...
  function buildInteriorSeats() {
    if (!interiorGroupRef) {
      console.error(
        "[SceneContents] buildInteriorSeats called before interiorGroupRef — aborting",
      )
      return
    }
  ...
  async function enterInterior() {
    ...
    const box =
      busInterior?.geometry.busBounds ??
      new THREE.Box3().setFromObject(exteriorGroupRef ?? new THREE.Group())
    const centre = new THREE.Vector3()
    const size = new THREE.Vector3()
    box.getCenter(centre)
    box.getSize(size)
