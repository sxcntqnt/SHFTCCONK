<script lang="ts">
  ...
  function handlePointerUp(e: PointerEvent, id: string) {
    ...
    if (Math.abs(deltaX) > 120 || velocity > VELOCITY_THRESHOLD)
...
<div
  class="fixed top-6 right-6 flex flex-col gap-3 z-[100] w-full max-w-[320px] pointer-events-none"
>
  {#each $notifications as notif (notif.id)}
    ...
    >
      <div class="flex items-start gap-3">
        <span
          class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-lg"
          >{icons[notif.type]}</span
        >
        <div class="flex-1 min-w-0">
          <h4 class="text-sm font-semibold text-gray-900 truncate">
            {notif.title}
          </h4>
          <p class="text-xs leading-relaxed text-gray-600 mt-0.5 truncate">
            {notif.message}
          </p>
        </div>
        <button
      ...
      <div
        ...
        style="
          width: {notif.isHovered
          ? `${(notif.remaining! / (notif.duration ?? 5000)) * 100}%`
          : '0%'};
