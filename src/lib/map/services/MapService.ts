...
export function filterMarkersByRadius(markers: MapMarker[], center: Coordinates, radius: number) {
  return markers.filter(marker => distanceBetween(marker.position, center) <= radius);
}
