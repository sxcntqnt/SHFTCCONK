...
export async function resolveUserState(
  ...
  const isGuest = !hasActiveActor || (profile as any).onboarding_status === 'GUEST'
  const isVerified = hasActiveActor && (profile as any).onboarding_status === 'ACTIVE'
...
supabase
  ...
  const activeContexts: ActorContext[] = actors.map(actor => {
    ...
    return {
      ...
      driverAssignment:    driverAssignments.find(d => d.actor_id === actor.id) ?? null,
      conductorAssignment: conductorAssigns.find(c => c.actor_id === actor.id) ?? null,
      fleetOwnership:      fleetOwnership.filter(f => f.actor_id === actor.id),
      stageAssignments:    stageAssignments.filter(s => s.operator_id === actor.id),
    }
  })
