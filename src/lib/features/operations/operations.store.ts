...
export const routeSummaries = derived(
  operationsStore,
  ($s) => $s.routeSummaries,
)
...
export const tripsInProgress = derived(operationsStore, ($s) =>
  $s.activeTrips.filter((t) => t.status === "IN_PROGRESS"),
)

export const delayedTrips = derived(operationsStore, ($s) =>
  $s.activeTrips.filter((t) => t.status === "DELAYED"),
)

export const boardingTrips = derived(operationsStore, ($s) =>
  $s.activeTrips.filter((t) => t.status === "BOARDING"),
)

export const openIncidents = derived(operationsStore, ($s) =>
  $s.incidents.filter((i) => i.status === "OPEN"),
)

/** Critical or escalated — drives the alert badge count */
export const urgentIncidents = derived(operationsStore, ($s) =>
  $s.incidents.filter(
    (i) =>
...
export const driversOnDuty = derived(operationsStore, ($s) =>
  $s.driverShifts.filter((d) => d.shiftState === "on_duty"),
)

export const driversOnBreak = derived(operationsStore, ($s) =>
  $s.driverShifts.filter((d) => d.shiftState === "on_break"),
)
...
export const routesWithNoCoverage = derived(operationsStore, ($s) =>
  $s.routeSummaries.filter((r) => r.health === "NO_COVERAGE"),
)

export const passengersInTransit = derived(operationsStore, ($s) =>
  $s.activeTrips
    .filter((t) => t.status === "IN_PROGRESS")
    .reduce((sum, t) => sum + t.passengerCount, 0),
)

export const pendingDispatches = derived(operationsStore, ($s) =>
  $s.dispatchQueue.filter((d) => d.status === "PENDING"),
)
...
export const opsSummary = derived(operationsStore, ($s) => ({
  ...
  incidentsCritical: $s.incidents.filter(
    (i) => i.severity === "CRITICAL" && i.status !== "RESOLVED",
  ).length,
  driversOnDuty: $s.driverShifts.filter((d) => d.shiftState === "on_duty")
    .length,
  driversOnBreak: $s.driverShifts.filter((d) => d.shiftState === "on_break")
    .length,
  pendingDispatches: $s.dispatchQueue.filter((d) => d.status === "PENDING")
    .length,
...
function assertTenant(orgId: string, table: string): void {
  if (_currentOrgId && orgId !== _currentOrgId) {
    console.error(
      `[ops:${table}] Tenant violation: got ${orgId}, expected ${_currentOrgId}`,
    )
      ...
      (payload) => {
        operationsStore.update((s) => {
          ...
          if (payload.eventType === "DELETE") {
            list = list.filter(
              (d) =>
                d.actorId !== (payload.old as { actor_id: string }).actor_id,
            )
          } else {
...
export async function destroyOperations(
  supabase: SupabaseClient,
): Promise<void> {
  await _teardown(supabase)
  _currentOrgId = null
...
export const getTripById = (id: string) =>
  get(operationsStore).activeTrips.find((t) => t.id === id)
export const getIncidentsByVehicle = (vehicleId: string) =>
  get(operationsStore).incidents.filter((i) => i.vehicleId === vehicleId)
export const getShiftByDriver = (actorId: string) =>
  get(operationsStore).driverShifts.find((d) => d.actorId === actorId)
export const getDispatchByVehicle = (vehicleId: string) =>
  get(operationsStore).dispatchQueue.filter((d) => d.vehicleId === vehicleId)
export const getRouteSummary = (routeId: string) =>
  get(operationsStore).routeSummaries.find((r) => r.routeId === routeId)

// ── Route summary computation ─────────────────────────────────────────────────

function computeRouteSummaries(
  trips: ActiveTrip[],
  orgId: string,
): RouteSummary[] {
  ...
  for (const trip of trips) {
    ...
    else if (s.health !== "DELAYED" && trip.status === "IN_PROGRESS")
      s.health = "ON_TIME"
    else if (s.health === "NO_COVERAGE" && trip.status === "SCHEDULED")
      s.health = "IDLE"

    if (!s.lastActivity || trip.updatedAt > s.lastActivity)
      s.lastActivity = trip.updatedAt
  }
...
function mapTrip(row: Record<string, unknown>): ActiveTrip {
  ...
  return {
    ...
    conductorActorId: row.conductor_actor_id
      ? String(row.conductor_actor_id)
      : undefined,
...
function mapDispatch(row: Record<string, unknown>): DispatchItem {
  ...
  return {
    ...
    assignedDriverId: row.assigned_driver_id
      ...
      : undefined,
    assignedConductorId: row.assigned_conductor_id
      ? String(row.assigned_conductor_id)
      : undefined,
