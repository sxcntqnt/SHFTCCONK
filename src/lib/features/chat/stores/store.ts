...
export async function fetchContacts(username: string): Promise<void> {
  ...
  try {
    const res: AxiosResponse<ContactsResponse> = await axios.get(
      `${API}/contact-list?username=${encodeURIComponent(username)}`,
    )
...
export async function loadMessages(
  ...
  try {
    ...
    const res: AxiosResponse<MessagesResponse> = await axios.get(
      `${API}/messages?username=${encodeURIComponent(user)}&${params.toString()}`,
    )
