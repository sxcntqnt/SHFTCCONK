...
export const CREW_INCENTIVE_RATE  = 0.10
export const PLATFORM_EXCESS_RATE = 0.80
 ...
 */
export function splitReservationFee(feeKes = RESERVATION_FEE.totalFeeKes): ReservationFeeSplit {
...
export function createLedgerEntry(
  ...
  return { ...params, id: uuidv4(), date: params.date ?? new Date().toISOString() }
}
...
export function createMpesaEntry(
  ...
  return createLedgerEntry({
    ...
    type: 'MPESA_COLLECTION', amount, reference: mpesaRef, mpesaPhone,
  })
}
...
export function createReservationFeeEntries(
  ...
  return [
    createLedgerEntry({ ...base, type: 'PLATFORM_SHARE',  amount: split.platform  }),
    createLedgerEntry({ ...base, type: 'DRIVER_SHARE',    amount: split.driver    }),
    createLedgerEntry({ ...base, type: 'CONDUCTOR_SHARE', amount: split.conductor }),
  ]
}
...
export function totalCollected(entries: LedgerEntry[]): number {
  return entries
    .filter((e) => e.type === 'MPESA_COLLECTION' || e.type === 'CASH_COLLECTION')
