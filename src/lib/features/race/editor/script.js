...
ca.onmousedown = function (e) {
  ...
  if (sel == 4) eraseL(gridX(mouse.cur.x), gridY(mouse.cur.y))
}
...
ca.onmousemove = function (e) {
  ...
  if (sel == 3 && mouse.down)
    arrows[arrows.length - 1].angle = Math.atan2(
      mouse.start.y - mouse.cur.y,
      mouse.start.x - mouse.cur.x,
    )
  if (sel == 4 && mouse.down) eraseL(gridX(mouse.cur.x), gridY(mouse.cur.y))
}
...
function eraseL(x, y) {
  for (var i = 0; i < walls.length; i++)
    if (
      Math.hypot(walls[i].start.x - x, walls[i].start.y - y) < 1 ||
  ...
  for (var i = 0; i < start.length; i++)
    if (
      Math.hypot(start[i].start.x - x, start[i].start.y - y) < 1 ||
