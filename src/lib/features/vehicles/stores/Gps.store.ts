...
export const allPositions = derived(
  gpsStore,
  ($s) => [...$s.locations.values()],
)

/** Vehicles with a valid fix */
export const activePositions = derived(
  gpsStore,
  ($s) => [...$s.locations.values()].filter((v) => v.fixStatus !== 'NO_FIX'),
)

/** Vehicles currently in rain */
export const vehiclesInRain = derived(
  gpsStore,
  ($s) =>
...
export async function initGps(
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('vehicle_id, organization_id, lat, lng, speed, heading, altitude, satellites, fix_status, hdop, timestamp, metadata')
